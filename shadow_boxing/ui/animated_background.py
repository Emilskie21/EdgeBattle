...
class PulsingSolidBackground:
    """Fallback when no game_background file loads — cheap animated fill."""

    def __init__(self) -> None:
        self._phase = 0.0

    def update(self, dt_ms: float) -> None:
        self._phase = (self._phase + dt_ms * 0.00012) % (2.0 * math.pi)

    def draw(self, screen: pygame.Surface) -> None:
        p = 0.5 + 0.5 * math.sin(self._phase)
        r = int(16 + 14 * p)
        g = int(12 + 10 * (1.0 - p))
        b = int(30 + 22 * p)
        screen.fill((r, g, b))
