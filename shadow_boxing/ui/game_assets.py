...
from shadow_boxing.constants import SCREEN_HEIGHT, SCREEN_WIDTH
from shadow_boxing.ui.animated_background import PulsingSolidBackground, load_background_from_path
from shadow_boxing.ui.assets_manifest import load_manifest, resolve_asset
...
def load_packaged_surfaces() -> dict[str, Any]:
    ...
    if not m:
        out["game_bg"] = PulsingSolidBackground()
        return out
    ...
    if isinstance(game_rel, str):
        ...
        out["game_bg"] = bg if bg is not None else PulsingSolidBackground()
    else:
        out["game_bg"] = PulsingSolidBackground()
