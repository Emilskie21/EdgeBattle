...
class PoseTracker:
    ...
    def _resolve_calibration_path(self) -> Path:
        """
        Your standalone calibration script saved to:
          `c:\\Users\\Master\\Documents\\booth2\\head_pose_calibration.json`
        but the game runs from `booth/`.
        """
        project_root = Path(__file__).resolve().parents[1]  # .../booth
        candidate_paths = [
            project_root.parent / "booth2" / "head_pose_calibration.json",
            project_root / "data" / "head_pose_calibration.json",
        ]
        for p in candidate_paths:
            if p.exists():
                return p
        # Default to booth's data folder if missing.
        return Path(__file__).resolve().parents[1] / "data" / "head_pose_calibration.json"
    ...
    def _resolve_model_path(self) -> Path:
        project_root = Path(__file__).resolve().parents[1]  # .../booth
        model_path = project_root / "data" / "face_landmarker.task"
    ...
    def update(self) -> PoseTrackingResult:
        ...
        if not self._neutral_calibrated:
            ...
            return PoseTrackingResult(camera_ok=True, detection_ok=True)
